...
export default function Login() {
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:5000/api/login', { login, senha }, { withCredentials: true });
            if (response.data.success) {
                const cargoUsuario = response.data.data.cargo; 
                    ...
                    <form onSubmit={handleLogin} style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <div className="input-container">
                            <div className="input-wrapper">
                                ...
                                <input type="text" placeholder="E-mail ou ID" className="input-field" value={login} onChange={e => setLogin(e.target.value)} required />
                            </div>
